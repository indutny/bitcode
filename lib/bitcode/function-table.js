...
const kSerializeInstruction = Symbol('serializeInstruction');
const kSerializeFunction = Symbol('serializeFunction');
...
class FunctionTable {
  ...
  [kSerializeInstruction](instr, table, stream) {
    if (instr.type === 'ret') {
      ...
      stream.writeUnabbrevRecord(RECORD.FUNCTION_INST_BR, [
        this.getBlockOffset(table, instr.operands[0])
      ]);
    } else {
  ...
  [kSerializeFunction](fn, stream) {
    ...
    blocks.list.forEach((block) => {
      block[kInstructions].forEach((instr) => {
        this[kSerializeInstruction](instr, {
          instructions,
          blocks
        }, stream);
      });
  ...
  serializeTo(stream) {
    ...
    this.fns.forEach((entry) => {
      if (entry.body !== null)
        this[kSerializeFunction](entry, stream);
    });
    stream.endBlock();
  }
}
