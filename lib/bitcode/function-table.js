...
class FunctionTable {
  ...
  serializeInstr(instr, table, stream) {
    if (instr.type === 'ret') {
      ...
      assert.strictEqual(instr.operands.length, 3,
        'binop should have exactly three operands (subtype, left, right)');
      assert(BINOP.hasOwnProperty(instr.operands[0]),
        'Invalid binop subtype: ' + instr.operands[0]);

      // TODO(indunty): support optimization flags
      const ops = [
        this.getInstrOffset(table, instr, instr.operands[1]),
        this.getInstrOffset(table, instr, instr.operands[2]),
        BINOP[instr.operands[0]]
      ];
  ...
  serializeFn(fn, stream) {
    ...
    signature.params.forEach((_, i) => {
      instructions.set(body.param(i), lastInstr++);
    });
    ...
    blocks.list.forEach((block) => {
      block[kInstructions].forEach((instr) => {
        ...
        instructions.set(instr, lastInstr);
        if (!instr.isVoid)
          lastInstr++;
      });
    });
