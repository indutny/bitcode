...
class Block {
  constructor(id, abbrVBR, reservedLen, info = null, parent = null) {
    this.id = id;
...
class LLBitStream extends BitStream {
  constructor() {
    ...
    // Stack of blocks
    this[kStack] = [ new Block(null, ROOT_ABBR_VBR, null) ];
  ...
  enterBlock(id, newAbbrVBR = null) {
    const stack = this[kStack];
    const parent = stack[stack.length - 1];

    if (!newAbbrVBR)
      newAbbrVBR = parent.abbrVBR;
    ...
    const res = new Block(id, newAbbrVBR, {
      offset,
  ...
  writeRecord(id, code, values) {
    ...
    const encodeOp = (op, value) => {
      ...
      } else if (op.type === 'array' && op.valueOp.type === 'char6') {
